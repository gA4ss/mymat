 ...
 */
template <class T>
Matrix<number_t> row_echelon_form(const Matrix<T>& mat) {
  if (mat.size() == 0) { matrix_exception("%s", "Size is 0"); }
...
__find_pivot:
    ...
    // 如果不为1，则先变为1。
    //
    if (!math::near<number_t>(_mat[pivotal_row][pivot], 1)) {
      //
      // 如果主元非1，则当前行乘以一个与主元互为倒数的k。
      //
      number_t p = _mat[pivotal_row][pivot];
    ...
    for (size_t i = pivotal_row+1; i < number_of_rows; i++) {
      ...
      //
      number_t i_pivot = _mat[i][pivot];
      for (size_t k = 0; k < number_of_columns; k++) {
        number_t v1 = _mat[pivotal_row][k] * i_pivot;
