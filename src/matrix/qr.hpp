 ...
 */
template <class T>
std::vector<std::vector<T>> __convert(const std::vector<std::vector<T>> &mat)
{
  size_t m = mat.size(), n = mat[0].size();
  std::vector<std::vector<T>> _mat(m);
  for (size_t i = 0; i < m; i++)
  {
    _mat[i].resize(n);
    for (size_t j = 0; j < n; j++)
      _mat[i][j] = static_cast<T>(mat[i][j]);
  }
  return _mat;
}
...
std::pair<Matrix<T>, Matrix<T>> qr(const Matrix<T> &mat)
{
  ...
  std::vector<std::vector<T>> q(m), r(m);
  for (size_t i = 0; i < m; i++)
  {
    q[i].resize(m);
    r[i].resize(n);
