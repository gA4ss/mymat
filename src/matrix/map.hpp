...
public:
  ...
  virtual bool condition(const Matrix<T>& mat, size_t i, size_t j) {
    return true;
  }

  virtual bool conditions(const std::vector<Matrix<T> >& mats, size_t i, size_t j) {
    return true;
  }
  ...
  int invoke(size_t i, size_t j, T& output) {
    if (!condition(i, j)) return 1;
    output = call(i, j);
    return 0;
  }

  int invoke(const Matrix<T>& mat, size_t i, size_t j, T& output) {
    if (!condition(mat, i, j)) return 1;
    output = call(mat, i, j);
    return 0;
  }

  int invokes(const std::vector<Matrix<T> >& mats, size_t i, size_t j, T& output) {
    if (!conditions(mats, i, j)) return 1;
...
Matrix<T> map(const Matrix<T>& mat, const MapFunction<T>& callback) {
  ...
  for (size_t i = 0; i < r; i++) {
    _mat[i].resize(c);
    for (size_t j = 0; j < c; j++) {
      ret = callback.invoke(mat, i, j, input);
      if (ret == 0) {
        _mat[i][j] = input;
      } else if (ret == -1) {
        break;
...
Matrix<T> maps(const std::vector<Matrix<T> >& mats, const MapFunction<T>& callback) {
  ...
  for (size_t i = 0; i < r; i++) {
    _mat[i].resize(c);
    for (size_t j = 0; j < c; j++) {
      ret = callback.invokes(mats, i, j, input);
      if (ret == 0) {
        _mat[i][j] = input;
      } else if (ret == -1) {
        break;
