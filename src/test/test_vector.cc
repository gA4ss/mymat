...
int main(int argc, char* argv[]) {
  ...
  Vector<int> tvec = transposition(vec1);
  std::cout << "vector'T = " << tvec.str() << std::endl;
  int d = dot(vec1, tvec);
  std::cout << "dot(vec1, tvec) = " << std::to_string(d) << std::endl;
  std::vector<int> value2 = {7,8,1,0,5,4,202};
  Vector<int> vec2(value2);
  std::cout << "add(vec1,vec2) = " << add(vec1, vec2).str() << std::endl;
  std::cout << "sub(vec2,vec1) = " << sub(vec2, vec1).str() << std::endl;
  std::cout << "mul(vec1,vec2) = " << mul(vec1, vec2).str() << std::endl;
  return 0;
}